...
export default function Home() {
  ...
  return (
    <main style={{ background: "#0a0a0e", color: "#f8fafc" }} className="mx-auto flex min-h-screen w-full max-w-4xl flex-col px-5 py-10 sm:py-16">
      ...
      <motion.header initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center text-white">
        <span className="mb-4 inline-flex items-center gap-2 rounded-full border border-cyan-400/30 bg-cyan-900/30 px-4 py-1 text-xs font-medium text-cyan-200">
      ...
      <footer className="mt-16 text-center text-xs text-white-faint">Not affiliated with YouTube. Download only content you have the right to.</footer>
    </main>
  );
}
